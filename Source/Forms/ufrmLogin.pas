...
uses
  ...
  FMX.Controls.Presentation, FMX.StdCtrls, FMX.Objects, FMX.Ani, FMX.Layouts,Model.Connection;
...
type
  ...
  private
    ...
    procedure SetFIndexConn(const Value: Longint);
  public
    { Public declarations }
    Property FIndexConn : Longint read FFIndexConn write SetFIndexConn;
  ...
  FIndexConn    := -1;
    Try
     FIndexConn  := Model.Connection.Connected('','');
      ...
      on E:Exception do
        begin
         ShowMessage( 'Erro ao desconectar o Banco : ' + E.Message);
          Abort;
        end;
    End;
...
procedure TFrmLogin.SetFIndexConn(const Value: Longint);
begin
  FFIndexConn := Value;
end;
